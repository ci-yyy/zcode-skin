...
## 1.2.5（2026-08-31）背景图全透模式

- **hero 主题（背景图/渐变）的 `backgroundAlt` 强制全透明**：该变量挂在根容器
  `bg-background-alt` 上铺满全窗，主题原值多为半透明深色（如折纸 `#201f1e8c`），
  等于在背景图上蒙 55% 黑纱——这是 hero 主题一直偏灰偏暗的根源。
  透明化后侧栏/会话列表/对话区/右栏的背景图 100% 直出，仅输入框保留实底（可读性）
- 卡片/弹窗/输入框用的 card/popover/input 等变量不受影响（原值实底）
- 实测折纸：五区域四透 100%，输入区 96% 实底；126/126 测试通过
- 仅改 `lib/theme.mjs` 的 `buildSkinCss`；若某主题想恢复蒙层观感，
  需回退本改动（引擎对 hero 主题的 backgroundAlt 强制透明）
