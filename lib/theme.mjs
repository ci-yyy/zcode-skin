...
export async function buildSkinCss(theme, themeDir) {
  ...
  const colors = theme.colors ?? {};
  // hero 主题（背景图/渐变）里 backgroundAlt 是铺满全窗的蒙层变量
  // （根容器 bg-background-alt）：主题原值多为半透明深色（如 #201f1e8c），
  // 等于在背景图上蒙 55% 黑纱。透明化让界面直接透图——卡片/输入框的局部
  // 实底（card/input 原值）不受影响，可读性由它们保障
  const effective = { ...colors };
  if (hero && effective.backgroundAlt) {
    effective.backgroundAlt = "#00000000";
  }
  const declarations = Object.entries(VAR_MAP)
    .filter(([key]) => effective[key])
    .map(([key, cssVar]) => `  ${cssVar}: ${effective[key].trim()} !important;`);
