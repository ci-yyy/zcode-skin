...
// 图片最长边超过 2560 时顺手降采样成 JPEG 回传（hero 从几 MB 降到几百 KB，
// 注入 CSS 才不会顶到体积上限）；PNG 带透明度的例外，缩放后保持 PNG。
export function extractPaletteScript(dataUrl) {
  return `(async () => {
    try {
      ...
      // 降采样：最长边 > 2560 或原图超过 2MB 时缩到 2560 存成 JPEG（省 80% 以上体积）
      let hero = null;
      const MAX_EDGE = 2560;
      const needScale = Math.max(img.width, img.height) > MAX_EDGE;
      // 注意判长度用 img.src（就是下面这个 dataUrl），别把 base64 再嵌一遍进脚本
      if (needScale || img.src.length > 2 * 1024 * 1024) {
        const scale = needScale ? MAX_EDGE / Math.max(img.width, img.height) : 1;
        ...
        // 检测透明度（扫全图，透明区可能不在左上角）：有 alpha 的图保持 PNG，否则转 JPEG
        const alphaData = bctx.getImageData(0, 0, w, h).data;
        let hasAlpha = false;
        for (let i = 3; i < alphaData.length; i += 4) {
          if (alphaData[i] < 250) { hasAlpha = true; break; }
        }
        hero = hasAlpha
          ? { dataUrl: big.toDataURL("image/png"), ext: ".png" }
          : { dataUrl: big.toDataURL("image/jpeg", 0.85), ext: ".jpg" };
      }
      return { ok: true, accent, secondary, avg: toHex(sr / n, sg / n, sb / n), avgL: totalL / n, hero };
    } catch (error) {
...
export async function createThemeFromImage({ session, imagePath, name, id, appearance = "auto", force = false, themesRoot }) {
  ...
  // 页面返回了降采样图（原图超大）→ 存它，hero 从几 MB 降到几百 KB
  let heroFile;
  if (palette.hero?.dataUrl?.startsWith("data:")) {
    const m = palette.hero.dataUrl.match(/^data:[^,]+,([A-Za-z0-9+/=]+)$/);
    if (!m) throw new Error("降采样图回传格式异常");
    heroFile = `hero${palette.hero.ext || ".jpg"}`;
    await mkdir(destDir, { recursive: true });
    await writeFile(join(destDir, heroFile), Buffer.from(m[1], "base64"));
  } else {
    // 原图不大：原样拷贝，保真
    heroFile = `hero${ext}`;
    await mkdir(destDir, { recursive: true });
