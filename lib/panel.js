...
(function () {
  ...
  // 请求超时：守护进程挂起时面板不会永久锁死在 applying 状态。
  // 默认 5 秒；上传建主题要走 12MB 传输 + 页面取色 + 写主题目录，慢机器上
  // 会超 5 秒——超时面板会误报失败而守护进程实际把主题建成了（状态分裂），
  // 所以上传路径用 opts.timeoutMs 放宽
  function fetchJson(url, opts) {
    var timeoutMs = (opts && opts.timeoutMs) || 5000;
    var finalOpts = opts ? Object.assign({}, opts) : {};
    delete finalOpts.timeoutMs; // 传给 fetch 前摘掉自定义字段
    var controller = typeof AbortController === "function" ? new AbortController() : null;
    if (controller) {
      finalOpts.signal = controller.signal;
      setTimeout(function () { controller.abort(); }, timeoutMs);
    }
    return fetch(url, finalOpts).then(function (res) { return res.json(); });
  }
  ...
  launcher.addEventListener("click", function (e) {
    e.stopPropagation();
    // Cmd/Ctrl+点击 = 收起/恢复小圆点；普通点击 = 开关面板
    ...
    fetchJson(API + "/css/" + encodeURIComponent(dir))
      ...
      .catch(function (e) {
        currentEl.textContent = "❌ " + (
          e && e.name === "AbortError" ? "连接超时（守护进程没响应）"
          : e && e.message ? e.message : "切换失败");
  ...
  function uploadImage(file) {
    ...
    if (file.size > 12 * 1024 * 1024) {
      currentEl.textContent = "❌ 图片超过 12MB";
      setTimeout(updateCurrentLabel, 2000);
      return;
    }
    applying = true;
    ...
    fetchJson(API + "/upload-theme", { method: "POST", body: form, timeoutMs: 30000 })
      .then(function (data) {
        if (!data.ok) throw new Error(data.error || "生成失败");
        return fetchJson(API + "/css/" + encodeURIComponent(data.dir), { timeoutMs: 15000 })
          .then(function (cssData) {
            if (!cssData.ok) throw new Error(cssData.error || "生成主题失败");
            swapSkin(cssData.css, cssData.id);
            currentDir = data.dir;
            // 记住这次选择（否则 ZCode 刷新后守护进程会恢复成旧主题）
            return fetchJson(API + "/applied/" + encodeURIComponent(data.dir), { method: "POST" });
          })
          .then(function () {
