...
// 页面目标分类：
//   main  = ZCode 主窗口（file:// 协议 + out/renderer/index.html）
//   panel = 电脑控制权限确认弹窗（cua-permission-panel.html，不注入）
//   unknown = 其他页面（如加载中、内部工具页、网页）
// main 判定必须带 file:// 前缀：http(s) 网页也能构造出撞名的 URL，不能注入它
export function classifyTargets(targets) {
  ...
  for (const target of Array.isArray(targets) ? targets : []) {
    ...
    const kind = url.includes("cua-permission-panel")
      ? "panel"
      : url.startsWith("file://") && url.includes("out/renderer/index.html")
        ? "main"
        : "unknown";
...
// 挑出主窗口。找不到精确匹配时，若恰好只有一个 file:// 协议的非弹窗页面，也认它
// （容错 ZCode 改路径）；http(s)/扩展页面绝不当回退目标——那是浏览器网页，注入越界。
export function pickMainWindow(pages) {
  ...
  const usable = pages.filter((p) =>
    p.kind !== "panel" && p.webSocketDebuggerUrl
    && typeof p.url === "string" && p.url.startsWith("file://"));
